...
export class IngredientController {
    ...
    async searchIngredient(req: express.Request,res: express.Response){
        ...
        const quantity = req.query.quantity ? Number.parseInt(req.query.quantity as string) : undefined ; // number
        const price = req.query.price ? Number.parseInt(req.query.price as string) : undefined ; // number
    ...
    async getIngredientIdByName(req: express.Request, res: express.Response){
        ...
        console.log(name);
        const result = await IngredientService.getInstance().getIngredientIdByName(name);
        if(result === ApiErrorCode.notFound) {
            return res.status(404).end();
        }
        if(result === ApiErrorCode.invalidParameters) {
    ...
    async updateIngredient(req: express.Request, res: express.Response) {
        const id = req.params.id;
        const data = req.body;
        const result = await IngredientService.getInstance().updateIngredient(id,data);
        if(result === ApiErrorCode.notFound) {
            return res.status(404).end();
        }
        if(result === ApiErrorCode.invalidParameters) {
