...
export class BurgerService {
    ...
    async searchBurgers(search: BurgerSearch): Promise<BurgerDocument[] | ApiErrorCode> {
        ...
        if(search.ingredient !== undefined){
            const ingredientId = await IngredientService.getInstance().getIngredientIdByName(search.ingredient);
    ...
    async createBurger(create: BurgerCreate): Promise<BurgerDocument | ApiErrorCode>{
        try {
    ...
    async updateBurger(id:string, update: BurgerUpdate): Promise<BurgerDocument | ApiErrorCode> {
        if(!Types.ObjectId.isValid(id)) {
            return ApiErrorCode.invalidParameters;
        }
